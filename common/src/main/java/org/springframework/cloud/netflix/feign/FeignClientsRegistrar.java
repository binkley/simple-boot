...
public class FeignClientsRegistrar
        ...
        for (String basePackage : basePackages) {
            ...
            for (BeanDefinition candidateComponent : candidateComponents) {
                if (candidateComponent instanceof AnnotatedBeanDefinition) {
                    ...
                    AnnotationMetadata annotationMetadata = beanDefinition
                            .getMetadata();
                    Assert.isTrue(annotationMetadata.isInterface(),
                            "@FeignClient can only be specified on an interface");

                    BeanDefinitionHolder holder = createBeanDefinition(
                            annotationMetadata);
    ...
    private BeanDefinitionHolder createBeanDefinition(
        ...
        definition.addPropertyValue("logLevel", getLogLevel(attributes));

        String beanName = StringUtils.uncapitalize(
                className.substring(className.lastIndexOf(".") + 1));
        return new BeanDefinitionHolder(definition.getBeanDefinition(),
                beanName);
    }
    ...
    private void validate(Map<String, Object> attributes) {
        if (StringUtils.hasText((String) attributes.get("value"))) {
            Assert.isTrue(
                    !StringUtils.hasText((String) attributes.get("name")),
    ...
    private String getServiceId(Map<String, Object> attributes) {
        ...
        Assert.state(host != null,
                "Service id not legal hostname (" + name + ")");
        return name;
    }
    ...
    private Logger.Level getLogLevel(Map<String, Object> attributes) {
            ...
            protected boolean isCandidateComponent(
                ...
                if (beanDefinition.getMetadata().isIndependent()) {
                    ...
                    if (beanDefinition.getMetadata().isInterface() &&
                            ...
                            .equals(beanDefinition.getMetadata()
                                    .getInterfaceNames()[0])) {
                        try {
    ...
    protected Set<String> getBasePackages(
        ...
        Map<String, Object> attributes = importingClassMetadata
                .getAnnotationAttributes(
                        EnableFeignClients.class.getCanonicalName());
        ...
        for (Class<?> clazz : (Class[]) attributes
